...
Healthcheck
     ...
     ${auth}=    Create List    runtimeUser    ${POLICY_ADMIN_PASSWORD}
     Log    Creating session http://${POLICY_RUNTIME_ACM_IP}:6970
     ${session}=    Create Session      ACM  http://${POLICY_RUNTIME_ACM_IP}:6970   auth=${auth}
     ${headers}=  Create Dictionary     Accept=application/json    Content-Type=application/json
     ${resp}=   GET On Session     ACM  /actuator/health     headers=${headers}
