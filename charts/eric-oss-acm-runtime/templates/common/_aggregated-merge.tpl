...
{{- define "eric-oss-acm-runtime.aggregatedMerge" -}}
  {{- $merged := dict -}}
  {{- $context := .context -}}
  {{- $location := .location -}}
...
*/ -}}
{{- define "eric-oss-acm-runtime.mergeAnnotations" -}}
  {{- include "eric-oss-acm-runtime.aggregatedMerge" (dict "context" "annotations" "location" .location "sources" .sources) }}
{{- end -}}

{{- define "eric-oss-acm-runtime.mergeLabels" -}}
  {{- include "eric-oss-acm-runtime.aggregatedMerge" (dict "context" "labels" "location" .location "sources" .sources) }}
{{- end -}}

{{/*
Create a user defined label (DR-D1121-068, DR-D1121-060)
*/}}
{{ define "eric-oss-acm-runtime.config-labels" }}
  {{- $global := (.Values.global).labels -}}
  {{- $service := .Values.labels -}}
  {{- include "eric-oss-acm-runtime.mergeLabels" (dict "location" .Template.Name "sources" (list $global $service)) }}
{{- end }}

{{/*
Merged labels for Default, which includes Standard and Config
*/}}
{{- define "eric-oss-acm-runtime.labels" -}}
  {{- $standard := include "eric-oss-acm-runtime.standard-labels" . | fromYaml -}}
  {{- $config := include "eric-oss-acm-runtime.config-labels" . | fromYaml -}}
  {{- include "eric-oss-acm-runtime.mergeLabels" (dict "location" .Template.Name "sources" (list $standard $config)) | trim }}
{{- end -}}
